...
export default {
  ...
  mounted() {
    this.$store.dispatch("getArticle17Instance");
  ...
  data() {
    return {
      ...
      numberID: 'IPFSImage17',
      ownerHistoryNumber: 'ownerList17',
      amount: null,
      yourName: null,
      pending: false,
      success: false,
...
      artistName:'none',
  ...
  methods: {
    buyConvention(event) {
      ...
      this.$store.state.article17Instance().buyCRC(
        this.yourName,
        {
          gas: 400000,
        ...
        (err, result) => {
          if (err) {
            ...
            this.pending = true;
            let newOwner = this.$store.state.article17Instance().newOwner();
    ...
    ArticleName(event) {
      this.$store.state.article17Instance().conventionArticleName(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleNumber(event) {
      this.$store.state.article17Instance().conventionArticleNumber(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleContentEvent(event) {
      this.$store.state.article17Instance().conventionArticleContent(
        (error, result) => {
          if (!error) {
    ...
    currentWorthOfArticle(event) {
      const theCurrentWorth = new Promise((resolve, reject) => {
        this.$store.state.article17Instance().currentWorth((error, result) => {
          if (error) {
    ...
    ipfsNewEvent(event) {
      const theIPFSHash = new Promise((resolve, reject) => {
        this.$store.state.article17Instance().ipfsImageHash((error, result) => {
          if (error) {
    ...
    ownerCount(event) {
      const promise1 = new Promise((resolve, reject) => {
        this.$store.state.article17Instance().owners((error, result) => {
          if (error) {
    ...
    ownerPromise(event) {
      const _ownerCount = new Promise((resolve, reject) => {
        this.$store.state.article17Instance().getUsersCount((error, result) => {
          if (error) {
            console.log(error);
          ...
          const getArray = new Promise((resolve, reject) => {
              ...
              for (let i = 0; i < result; i++) {
                  const ownernames = new Promise((resolve, reject) => {
                      this.$store.state.article17Instance().getUser(i, (error, result) => {
                          if (error) {
...
        this.$store.state.article17Instance().artistName((error, result) => {
          if (error) {
