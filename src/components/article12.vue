...
export default {
  name: "article12",
  mounted() {
    console.log("dispatching getArticle12Instance");
    this.$store.dispatch("getArticle12Instance");
...
      artistName:'none'
  ...
  methods: {
    buyConvention(event) {
      ...
      this.$store.state.article12Instance().buyCRC(
        this.yourName,
        {
          gas: 400000,
        ...
        (err, result) => {
          if (err) {
            ...
            this.pending = true;
            let newOwner = this.$store.state.article12Instance().newOwner();
    ...
    ArticleName(event) {
      ...
      this.$store.state.article12Instance().conventionArticleName(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleNumber(event) {
      ...
      this.$store.state.article12Instance().conventionArticleNumber(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleContentEvent(event) {
      ...
      this.$store.state.article12Instance().conventionArticleContent(
        (error, result) => {
          if (!error) {
    ...
    currentWorthOfArticle(event) {
      ...
      const theCurrentWorth = new Promise((resolve, reject) => {
        this.$store.state.article12Instance().currentWorth((error, result) => {
          if (error) {
          ...
          resolve(result);
          this.$emit('current-worthtvelve', this.CurrentWorth)
    ...
    ipfsNewEvent(event) {
      ...
      const theIPFSHash = new Promise((resolve, reject) => {
        this.$store.state.article12Instance().ipfsImageHash((error, result) => {
          if (error) {
        ...
        document.getElementById("IPFSImage12").src = myURLBitch;
      });
    },
    ownerCount(event) {
      console.log("getting owner count");
      const promise1 = new Promise((resolve, reject) => {
        this.$store.state.article12Instance().owners((error, result) => {
          if (error) {
            console.log(err);
    ...
    ownerPromise(event) {
      ...
      const _ownerCount = new Promise((resolve, reject) => {
        this.$store.state.article12Instance().getUsersCount((error, result) => {
          if (error) {
            console.log(error);
          ...
          const getArray = new Promise((resolve, reject) => {
              ...
              for (let i = 0; i < result; i++) {
                  const ownernames = new Promise((resolve, reject) => {
                      this.$store.state.article12Instance().getUser(i, (error, result) => {
                          if (error) {
...
        this.$store.state.article12Instance().artistName((error, result) => {
          if (error) {
...
          }
          resolve(result);
          this.$emit('artist-nametvelve', this.artistName)
