...
export default {
  name: "article16",
  mounted() {
    console.log("dispatching getArticle16Instance");
    this.$store.dispatch("getArticle16Instance");
...
      artistName:'none'
  ...
  methods: {
    buyConvention(event) {
      ...
      this.$store.state.article16Instance().buyCRC(
        this.yourName,
        {
          gas: 400000,
        ...
        (err, result) => {
          if (err) {
            ...
            this.pending = true;
            let newOwner = this.$store.state.article16Instance().newOwner();
    ...
    ArticleName(event) {
      ...
      this.$store.state.article16Instance().conventionArticleName(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleNumber(event) {
      ...
      this.$store.state.article16Instance().conventionArticleNumber(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleContentEvent(event) {
      ...
      this.$store.state.article16Instance().conventionArticleContent(
        (error, result) => {
          if (!error) {
    ...
    currentWorthOfArticle(event) {
      ...
      const theCurrentWorth = new Promise((resolve, reject) => {
        this.$store.state.article16Instance().currentWorth((error, result) => {
          if (error) {
          ...
          resolve(result);
          this.$emit('current-worthsixteen', this.CurrentWorth)
    ...
    ipfsNewEvent(event) {
      ...
      const theIPFSHash = new Promise((resolve, reject) => {
        this.$store.state.article16Instance().ipfsImageHash((error, result) => {
          if (error) {
        ...
        document.getElementById("IPFSImage16").src = myURLBitch;
      });
    },
    ownerCount(event) {
      console.log("getting owner count");
      const promise1 = new Promise((resolve, reject) => {
        this.$store.state.article16Instance().owners((error, result) => {
          if (error) {
            console.log(err);
    ...
    ownerPromise(event) {
      ...
      const _ownerCount = new Promise((resolve, reject) => {
        this.$store.state.article16Instance().getUsersCount((error, result) => {
          if (error) {
            console.log(error);
          ...
          const getArray = new Promise((resolve, reject) => {
              ...
              for (let i = 0; i < result; i++) {
                  const ownernames = new Promise((resolve, reject) => {
                      this.$store.state.article16Instance().getUser(i, (error, result) => {
                          if (error) {
...
        this.$store.state.article16Instance().artistName((error, result) => {
          if (error) {
...
          }
          resolve(result);
          this.$emit('artist-namesixteen', this.artistName)
