...
<div class="flipControls">
    ...
    <div class="articleControls" v-bind:class="{ 'transparency' : isMouseOver }">
      <p class="gold" v-if="hasOwners">Current worth: {{CurrentWorth}} ETH</p>
          <p class="NoOwner" v-if="!hasOwners">This article has not been ratified by anyone yet. <i>Claim it for any price.</i></p>
  ...
  methods: {
    ...
    currentWorthOfArticle(event) {
      ...
      const theCurrentWorth = new Promise((resolve, reject) => {
        this.$store.state.contractInstance().currentWorth((error, result) => {
          if (error) {
