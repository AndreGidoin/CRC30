...
export default {
  ...
  mounted() {
    this.$store.dispatch("getArticle19Instance");
  ...
  data() {
    return {
      number: 19,
      textnumber: 'nineteen',
      numberID: 'IPFSImage19',
      ownerHistoryNumber: 'ownerList19',
      amount: null,
      yourName: null,
      pending: false,
      success: false,
...
      artistName:'none',
  ...
  methods: {
    buyConvention(event) {
      ...
      this.$store.state.article19Instance().buyCRC(
        this.yourName,
        {
          gas: 400000,
        ...
        (err, result) => {
          if (err) {
            ...
            this.pending = true;
            let newOwner = this.$store.state.article19Instance().newOwner();
    ...
    ArticleName(event) {
      this.$store.state.article19Instance().conventionArticleName(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleNumber(event) {
      this.$store.state.article19Instance().conventionArticleNumber(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleContentEvent(event) {
      this.$store.state.article19Instance().conventionArticleContent(
        (error, result) => {
          if (!error) {
    ...
    currentWorthOfArticle(event) {
      const theCurrentWorth = new Promise((resolve, reject) => {
        this.$store.state.article19Instance().currentWorth((error, result) => {
          if (error) {
    ...
    ipfsNewEvent(event) {
      const theIPFSHash = new Promise((resolve, reject) => {
        this.$store.state.article19Instance().ipfsImageHash((error, result) => {
          if (error) {
    ...
    ownerCount(event) {
      const promise1 = new Promise((resolve, reject) => {
        this.$store.state.article19Instance().owners((error, result) => {
          if (error) {
    ...
    ownerPromise(event) {
      const _ownerCount = new Promise((resolve, reject) => {
        this.$store.state.article19Instance().getUsersCount((error, result) => {
          if (error) {
            console.log(error);
          ...
          const getArray = new Promise((resolve, reject) => {
              ...
              for (let i = 0; i < result; i++) {
                  const ownernames = new Promise((resolve, reject) => {
                      this.$store.state.article19Instance().getUser(i, (error, result) => {
                          if (error) {
...
        this.$store.state.article19Instance().artistName((error, result) => {
          if (error) {
