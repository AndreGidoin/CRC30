<template>
  <div class="root-container">
    ...
    <div class="articleNumber" v-if="crcArticleChecked">
            <p v-if="crcArticleChecked"><span id="numerals">{{ crcArticleEvent }}</span></p>
      </div>
    <!-- Article Number END -->
    
    <!-- The Convention Article and Artwork -->
    <div class="Frontside">
      <h1 v-if="articleNameChecked">{{ ArticleNameEvent }}</h1>
      ...
      <!-- Owner Info -->
      <div class="theOwners" v-if="!hasOwners">
      ...
      <div class="theOwners" v-if="hasOwners">
          <p><b>{{ theCurrentOwner }} is the current owner of this contract</b></p>
        </div>

    </div>

    <!-- About the Artwork and ordering a print -->
    <div class="Backside">
    ...
    <div class="articleControls">
      <h4>Buy it for more than it's current worth and become a part owner in addition to receiving a printed version shipped free to you</h4>
      <div class="buing">
        <p>
          Enter your price:
    ...
    <!-- Stuff to delete -->
    <div class="buttonsfortest">
        <button v-on:click="allOwners">Check owners</button>
        <button v-on:click="testOwners">Test owners</button>
        <button v-on:click="ownerCount">Owner Count</button>
        <button v-on:click="getOwnerName">Get owner name</button>
        <button v-on:click="awaitOwners">Await function</button>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    setTimeout(this.myImageLoader, 200);
  ...
  methods: {
    ...
    // Getting the array of all the owners of the contract
    allOwners(event) {
      console.log("Getting all owners from array");
      const theOwners = new Promise((resolve, reject) => {
        this.$store.state.contractInstance().getUsersCount((error, result) => {
          if (error) {
            console.error(err + " cant get the getUsersCount");
            reject(new Error("getUsersCount function didnt work properly"));
          } else {
            console.log(
              result + " is the amount of owners of this article/contract"
            );
          }
        });
        resolve(result);
      });
      theOwners.then(result => {
        console.log("the then thing");
        for (let i = 0; i < result; i++) {
          console.log(this.$store.state.contractInstance().getUser[i]);
        }
        console.log(data);
      });
    },
    // this is where I left off July 10th
    testOwners(event) {
      const promise1 = new Promise((resolve, reject) => {
        this.$store.state.contractInstance().getUsersCount((error, result) => {
          if (error) {
            console.log(err);
            reject(new Error("testOwners function went wrong"));
          } else {
            console.log(result);
            resolve(result);
          }
      ...
      promise1.then(result => {
        ...
        function step(i) {
          if (i === i) {
            this.$store.state
              .contractInstance()
              .getUser([i], (error, result) => {
                if (error) {
                  console.log(err);
                } else {
                  console.log(i);
                  allOwnerNames.push(result);
                  console.log(result);
                  step(i + 1);
                }
              });
    ...
    getOwnerName(event) {
      console.log("getting name of owner number 2");
      const OwnerName = new Promise((resolve, reject) => {
        this.$store.state.contractInstance().getUser(2, (error, result) => {
          if (error) {
            console.log(error);
            reject(new Error("can get name of user number 2"));
          } else {
            console.log(result);
            console.log(result.slice(2, 9));
            console.log(JSON.stringify(result.slice(0)));
            resolve(result);
          }
        });
      });
    },
    async awaitOwners(event) {
      console.log("running awaitOwners");
      const count = await this.$store.state
        .contractInstance()
      ...
      for (let i = 0; i < count; i++) {
        const _ownername = await this.$store.state.contractInstance().getUser[
          i
        ];
        ownersarray.push(_ownername);
        console.log(_ownername[1]);
