...
export default {
  name: "article3",
  mounted() {
    console.log("dispatching getArticle3Instance");
    this.$store.dispatch("getArticle3Instance");
  ...
  methods: {
    buyConvention(event) {
      ...
      this.$store.state.article3Instance().buyCRC(
        this.yourName,
        {
          gas: 300000,
        ...
        (err, result) => {
          if (err) {
            ...
            this.pending = true;
            let newOwner = this.$store.state.article3Instance().newOwner();
    ...
    ArticleName(event) {
      ...
      this.$store.state.article3Instance().conventionArticleName(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleNumber(event) {
      ...
      this.$store.state.article3Instance().conventionArticleNumber(
        {
          gas: 30000,
          from: this.$store.state.web3.coinbase
        },
    ...
    crcArticleContentEvent(event) {
      ...
      this.$store.state.article3Instance().contentionArticleContent(
    ...
    currentWorthOfArticle(event) {
      ...
      const theCurrentWorth = new Promise((resolve, reject) => {
        this.$store.state.article3Instance().currentWorth((error, result) => {
          if (error) {
    ...
    ipfsNewEvent(event) {
      ...
      const theIPFSHash = new Promise((resolve, reject) => {
        this.$store.state.article3Instance().ipfsImageHash((error, result) => {
          if (error) {
        ...
        document.getElementById("IPFSImage3").src = myURLBitch;
      });
    },
    ownerCount(event) {
      console.log("getting owner count");
      const promise1 = new Promise((resolve, reject) => {
        this.$store.state.article3Instance().owners((error, result) => {
          if (error) {
            console.log(err);
    ...
    ownerPromise(event) {
      ...
      const _ownerCount = new Promise((resolve, reject) => {
        this.$store.state.article3Instance().getUsersCount((error, result) => {
          if (error) {
            console.log(error);
          ...
          const getArray = new Promise((resolve, reject) => {
              ...
              for (let i = 0; i < result; i++) {
                  const ownernames = new Promise((resolve, reject) => {
                      this.$store.state.article3Instance().getUser(i, (error, result) => {
                          if (error) {
    ...
    showOwnerHistoryList(event) {
        ...
        for (let i = 0; i < (this.ownersarray2.length); i++) {
            ...
            forPromise.then((result) => {
                ...
                document.getElementById("ownerHistory3").innerHTML = result;
                console.log(result);
            })
        }
    },
